...
class DataLoader:
    ...
    def get_batches_fn_timeseries(self, batch_size):
        ...
        for batch_i in range(0, len(event_images), batch_size):
            ...
            for i in range(batch_i, np.min([batch_i+batch_size, len(event_images)])):
                temp_event_image = np.copy(event_images[i][0:self.target_image_height_, 0:self.target_image_width_, :])
                temp_event_image = np.array([temp_event_image])
                temp_event_image = np.swapaxes(temp_event_image, 0, 3) #Channels become number of timesteps
                temp_event_image = np.flip(temp_event_image, axis=0) #Most recent sample is last
                inputs.append(temp_event_image)
